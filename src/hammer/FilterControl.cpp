...
BEGIN_MESSAGE_MAP(CFilterControl, CHammerBar)
	...
	ON_BN_CLICKED(IDC_EDITGROUPS, &CFilterControl::OnEditGroups)
#ifdef HAMMER2013_PORT_CORDONS
	ON_BN_CLICKED(IDC_NEW, &CFilterControl::OnNew)
	ON_BN_CLICKED(IDC_DELETE, &CFilterControl::OnDelete)
#endif
	ON_BN_CLICKED(IDC_MARKMEMBERS, &CFilterControl::OnMarkMembers)
	ON_BN_CLICKED(IDC_SHOW_ALL, &CFilterControl::OnShowAllGroups)
	ON_COMMAND_EX(IDC_VISGROUP_MOVEUP, &CFilterControl::OnMoveUpDown)
	ON_COMMAND_EX(IDC_VISGROUP_MOVEDOWN, &CFilterControl::OnMoveUpDown)
	ON_UPDATE_COMMAND_UI(IDC_GROUPS, &CFilterControl::UpdateControlGroups)
#ifdef HAMMER2013_PORT_CORDONS
	ON_UPDATE_COMMAND_UI(IDC_CORDONS, &CFilterControl::UpdateControl)
	ON_UPDATE_COMMAND_UI(IDC_NEW, &CFilterControl::UpdateControl)
#endif
	ON_UPDATE_COMMAND_UI(IDC_EDITGROUPS, &CFilterControl::UpdateControl)
	ON_UPDATE_COMMAND_UI(IDC_MARKMEMBERS, &CFilterControl::UpdateControl)
	ON_UPDATE_COMMAND_UI(IDC_SHOW_ALL, &CFilterControl::UpdateControl)
	ON_UPDATE_COMMAND_UI(IDC_VISGROUP_MOVEUP, &CFilterControl::UpdateControl)
	ON_UPDATE_COMMAND_UI(IDC_VISGROUP_MOVEDOWN, &CFilterControl::UpdateControl)		
	ON_NOTIFY(TCN_SELCHANGE, IDC_TAB1, &CFilterControl::OnSelChangeTab)
	ON_WM_ACTIVATE()
	ON_WM_SHOWWINDOW()
	ON_WM_SIZE()
	ON_WM_WINDOWPOSCHANGED()
	ON_REGISTERED_MESSAGE(g_uToggleStateMsg, &CFilterControl::OnListToggleState)
	ON_REGISTERED_MESSAGE(g_uLeftDragDropMsg, &CFilterControl::OnListLeftDragDrop)
	ON_REGISTERED_MESSAGE(g_uRightDragDropMsg, &CFilterControl::OnListRightDragDrop)
#ifdef HAMMER2013_PORT_CORDONS
	ON_REGISTERED_MESSAGE(g_uSelChangeMsg, &CFilterControl::OnListSelChange)
	ON_REGISTERED_MESSAGE(g_uKeyDownMsg, &CFilterControl::OnListKeyDown)
#ifdef SLE //// SLE NEW - double click to mark the group
	ON_REGISTERED_MESSAGE(g_uDoubleLClickMsg, &CFilterControl::OnListDblLClick)
#endif
#endif
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
