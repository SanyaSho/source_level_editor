...
BEGIN_MESSAGE_MAP(CTextureBrowser, CDialog)
	...
	ON_WM_SIZE()
	ON_CBN_SELENDOK(IDC_TEXTURESIZE, &CTextureBrowser::OnSelendokTexturesize)
	ON_WM_CLOSE()
	ON_WM_TIMER()
	ON_CBN_EDITCHANGE(IDC_FILTER, &CTextureBrowser::OnChangeFilterOrKeywords)
	ON_CBN_SELENDOK(IDC_FILTER, &CTextureBrowser::OnUpdateFiltersNOW)
#ifndef SLE_NO_TEXTURE_KEYWORDS
	ON_CBN_EDITCHANGE(IDC_KEYWORDS, &CTextureBrowser::OnChangeFilterOrKeywords)
	ON_CBN_SELENDOK(IDC_KEYWORDS, &CTextureBrowser::OnUpdateKeywordsNOW)
#endif

	ON_BN_CLICKED(IDC_FILTER_OPAQUE, &CTextureBrowser::OnFilterOpaque)
	ON_BN_CLICKED(IDC_FILTER_TRANSLUCENT, &CTextureBrowser::OnFilterTranslucent)
	ON_BN_CLICKED(IDC_FILTER_SELFILLUM, &CTextureBrowser::OnFilterSelfIllum)
	ON_BN_CLICKED(IDC_FILTER_SPECULAR, &CTextureBrowser::OnFilterSpecular)
	ON_BN_CLICKED(IDC_SHOW_ERROR, &CTextureBrowser::OnShowErrors)

	ON_BN_CLICKED(IDC_SHOW_ONLY_USED_TEXTURES, &CTextureBrowser::OnShowOnlyUsedTextures)
	ON_BN_CLICKED(IDC_MARK, &CTextureBrowser::OnMark)
	ON_BN_CLICKED(IDC_REPLACE, &CTextureBrowser::OnReplace)
	ON_BN_CLICKED(IDC_TEXTURES_OPEN_SOURCE, &CTextureBrowser::OnOpenSource)
	ON_BN_CLICKED(IDC_TEXTURES_RELOAD, &CTextureBrowser::OnReload)
	ON_MESSAGE(TWN_SELCHANGED, &CTextureBrowser::OnTexturewindowSelchange)
	ON_MESSAGE(TWN_LBUTTONDBLCLK, &CTextureBrowser::OnTextureWindowDblClk)
#ifdef SLE_TEXTUREBROWSER_FAVOURITES
	ON_BN_CLICKED(IDC_TOFAVS, &CTextureBrowser::OnAddCurrentMaterialToFavs)
	ON_BN_CLICKED(IDC_FROMFAVS, &CTextureBrowser::OnRemoveCurrentMaterialFromFavs)
	ON_BN_CLICKED(IDC_FAVSLIST, &CTextureBrowser::OnOpenMaterialConfig)
#endif
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
