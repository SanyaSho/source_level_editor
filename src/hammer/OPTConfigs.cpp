...
BEGIN_MESSAGE_MAP(COPTConfigs, CPropertyPage)
	...
	ON_BN_CLICKED(IDC_EDITCONFIGS, &COPTConfigs::OnEditconfigs)
	ON_BN_CLICKED(IDC_GDFILE_ADD, &COPTConfigs::OnGdfileAdd)
	ON_BN_CLICKED(IDC_GDFILE_EDIT, &COPTConfigs::OnGdfileEdit)
	ON_BN_CLICKED(IDC_GDFILE_REMOVE, &COPTConfigs::OnGdfileRemove)
	ON_CBN_SELCHANGE(IDC_CONFIGURATIONS, &COPTConfigs::OnSelchangeConfigurations)
	ON_BN_CLICKED(IDC_BROWSEMAPDIR, &COPTConfigs::OnBrowsemapdir)
	ON_BN_CLICKED(IDC_BROWSEGAMEEXEDIR, &COPTConfigs::OnBrowseGameExeDir)
	ON_BN_CLICKED(IDC_BROWSEMODDIR, &COPTConfigs::OnBrowseModDir)
	ON_BN_CLICKED(IDC_BROWSE_CORDON_TEXTURE, &COPTConfigs::OnBrowseCordonTexture)
	ON_MESSAGE( WM_SETTINGCHANGE, &COPTConfigs::OnSettingChange )
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
