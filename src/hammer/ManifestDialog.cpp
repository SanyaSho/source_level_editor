...
BEGIN_MESSAGE_MAP(CManifestListBox, CListBox)
	...
	//}}AFX_MSG_MAP
	ON_WM_LBUTTONDBLCLK()
	ON_COMMAND(ID_MOVESELECTIONTO_SUBMAP, &CManifestListBox::OnMoveSelectionToSubMap)
	ON_COMMAND(ID_MOVESELECTIONTO_NEWSUBMAP, &CManifestListBox::OnMoveSelectionToNewSubMap)
	ON_COMMAND(ID_VERSIONCONTROL_CHECKOUT, &CManifestListBox::OnVersionControlCheckOut)
	ON_COMMAND(ID_VERSIONCONTROL_CHECKIN, &CManifestListBox::OnVersionControlCheckIn)
	ON_COMMAND(ID_VERSIONCONTROL_ADD, &CManifestListBox::OnVersionControlAdd)
	ON_COMMAND(ID_INSERT_EMPTYSUBMAP, &CManifestListBox::OnInsertEmptySubMap)
	ON_COMMAND(ID_INSERT_EXISTINGSUBMAP, &CManifestListBox::OnInsertExistingSubMap)
	ON_COMMAND(ID_MANIFEST_PROPERTIES, &CManifestListBox::OnManifestProperties)
	ON_COMMAND(ID_MANIFEST_REMOVE, &CManifestListBox::OnManifestRemove)
END_MESSAGE_MAP()
