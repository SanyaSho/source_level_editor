...
BEGIN_MESSAGE_MAP(CEntityReportDlg, CDialog)
	...
	ON_BN_CLICKED(IDC_DELETE, &CEntityReportDlg::OnDelete)
	ON_BN_CLICKED(IDC_FILTERBYHIDDEN, &CEntityReportDlg::OnFilterbyhidden)
	ON_BN_CLICKED(IDC_FILTERBYKEYVALUE, &CEntityReportDlg::OnFilterbykeyvalue)
	ON_BN_CLICKED(IDC_FILTERBYTYPE, &CEntityReportDlg::OnFilterbytype)
	ON_EN_CHANGE(IDC_FILTERKEY, &CEntityReportDlg::OnChangeFilterkey)
	ON_EN_CHANGE(IDC_FILTERVALUE, &CEntityReportDlg::OnChangeFiltervalue)
	ON_BN_CLICKED(IDC_GOTO, &CEntityReportDlg::OnGoto)
	ON_BN_CLICKED(IDC_PROPERTIES, &CEntityReportDlg::OnProperties)
	ON_WM_TIMER()
#ifdef SLE //// SLE CHANGE - make Entity Report resizable
	ON_WM_SIZE()
#endif
	ON_CBN_EDITCHANGE(IDC_FILTERCLASS, &CEntityReportDlg::OnEditchangeFilterclass)
	ON_BN_CLICKED(IDC_FILTERBYCLASS, &CEntityReportDlg::OnFilterbyclass)
	ON_CBN_SELCHANGE(IDC_FILTERCLASS, &CEntityReportDlg::OnSelchangeFilterclass)
	ON_BN_CLICKED(IDC_RADIO2, &CEntityReportDlg::OnFilterbytype)
	ON_BN_CLICKED(IDC_RADIO3, &CEntityReportDlg::OnFilterbytype)
	ON_BN_CLICKED(IDC_EXACTVALUE, &CEntityReportDlg::OnExactvalue)
#ifdef SLE //// SLE NEW: Let us filter entities that do NOT have the specified value
	ON_BN_CLICKED(IDC_FILTERREVERSE, &CEntityReportDlg::OnFilterReverse)
#endif
	ON_LBN_SELCHANGE(IDC_ENTITIES, &CEntityReportDlg::OnSelChangeEntityList)
	ON_LBN_DBLCLK(IDC_ENTITIES, &CEntityReportDlg::OnDblClkEntityList)
	ON_WM_DESTROY()
	ON_WM_CLOSE()
	//}}AFX_MSG_MAP
END_MESSAGE_MAP()
...
#ifdef SLE
				pWorld->EnumChildrenOfInstance(ENUMMAPCHILDRENPROC(AddEntityToList), DWORD_PTR(pDlg), MAPCLASS_TYPE(CMapEntity), pEntity->GetKeyValue("file"));
#else
