...
BEGIN_MESSAGE_MAP(COP_Output, CObjectPage)
	...
	ON_BN_CLICKED(IDC_ADD,		&COP_Output::OnAdd)
	ON_BN_CLICKED(IDC_DELETE,	&COP_Output::OnDelete)
	ON_BN_CLICKED(IDC_COPY,		&COP_Output::OnCopy)
	ON_WM_SIZE()
	ON_BN_CLICKED(IDC_PASTE,	&COP_Output::OnPaste)
	ON_BN_CLICKED(IDC_MARK,		&COP_Output::OnMark)
	ON_BN_CLICKED(IDC_PICK_ENTITY, &COP_Output::OnPickEntity)
	ON_BN_CLICKED(IDC_PICK_ENTITY_PARAM,	&COP_Output::OnPickEntityParam)
	ON_CBN_SELCHANGE(IDC_EDIT_CONN_INPUT,	&COP_Output::OnSelChangeInput)
	ON_CBN_EDITUPDATE(IDC_EDIT_CONN_INPUT,	&COP_Output::OnEditUpdateInput)
	ON_CBN_SELCHANGE(IDC_EDIT_CONN_OUTPUT,	&COP_Output::OnSelChangeOutput)
	ON_CBN_EDITUPDATE(IDC_EDIT_CONN_OUTPUT, &COP_Output::OnEditUpdateOutput)
	ON_CBN_SELCHANGE(IDC_EDIT_CONN_PARAM,	&COP_Output::OnSelChangeParam)
	ON_CBN_EDITUPDATE(IDC_EDIT_CONN_PARAM,	&COP_Output::OnEditUpdateParam)
	ON_EN_CHANGE(IDC_EDIT_CONN_DELAY,		&COP_Output::OnEditDelay)
#ifdef SLE //// SLE CHANGE - replace Only Once with Times to Fire
	ON_EN_CHANGE(IDC_EDIT_CONN_TIMESTOFIRE, &COP_Output::OnEditTimesToFire)
#else
	ON_BN_CLICKED(IDC_EDIT_CONN_FIRE_ONCE, &COP_Output::OnFireOnce)
#endif
#ifdef SLE //// SLE NEW - Mark Button for entities in output argument param
	ON_BN_CLICKED(IDC_MARK_PARAM, &COP_Output::OnMarkParam)
#endif
	ON_BN_CLICKED(IDC_SHOWHIDDENTARGETS,	&COP_Output::OnShowHiddenTargetsAsBroken)
	ON_WM_DESTROY()
