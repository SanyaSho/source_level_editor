...
BEGIN_MESSAGE_MAP(CSoundBrowser, CDialog)
	...
	ON_WM_CLOSE()
	ON_CBN_EDITCHANGE(IDC_SOUND_FILTER, &CSoundBrowser::OnChangeFilter)
	ON_CBN_SELENDOK(IDC_SOUND_FILTER, &CSoundBrowser::OnUpdateFilterNOW)
	ON_CBN_SELCHANGE(IDC_SOUND_TYPE, &CSoundBrowser::OnSelchangeSoundType)
	ON_LBN_SELCHANGE(IDC_SOUND_LIST, &CSoundBrowser::OnSelchangeSoundList)
	ON_LBN_DBLCLK(IDC_SOUND_LIST, &CSoundBrowser::OnDblclkSoundList)
	ON_BN_CLICKED(IDC_PREVIEW, &CSoundBrowser::OnPreview)
	ON_BN_CLICKED(IDC_AUTOPLAY, &CSoundBrowser::OnAutoplay)
	ON_BN_CLICKED(IDC_STOPSOUND, &CSoundBrowser::OnBnClickedStopsound)
	ON_BN_CLICKED(IDC_REFRESH_SOUNDS, &CSoundBrowser::OnRefreshSounds)
	ON_WM_TIMER()
	ON_BN_CLICKED(IDC_OPEN_SOURCE, &CSoundBrowser::OnOpenSource)
#ifdef HAMMER2013_PORT_FMOD
	ON_WM_HSCROLL()
#endif
	//}}AFX_MSG_MAP
