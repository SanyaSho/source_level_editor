...
BEGIN_MESSAGE_MAP( CFaceEditDispPage, CPropertyPage )
	...
	ON_BN_CLICKED( ID_DISP_MASK_SELECT, &CFaceEditDispPage::OnCheckMaskSelect )
	ON_BN_CLICKED( ID_DISP_MASK_GRID, &CFaceEditDispPage::OnCheckMaskGrid )

	ON_BN_CLICKED( ID_DISP_NOPHYSICS_COLL, &CFaceEditDispPage::OnCheckNoPhysicsCollide )
	ON_BN_CLICKED( ID_DISP_NOHULL_COLL, &CFaceEditDispPage::OnCheckNoHullCollide )
	ON_BN_CLICKED( ID_DISP_NORAY_COLL, &CFaceEditDispPage::OnCheckNoRayCollide )

	ON_BN_CLICKED( ID_DISP_SELECT2, &CFaceEditDispPage::OnButtonSelect )
	ON_BN_CLICKED( ID_DISP_CREATE, &CFaceEditDispPage::OnButtonCreate )
	ON_BN_CLICKED( ID_DISP_DESTROY, &CFaceEditDispPage::OnButtonDestroy )
	ON_BN_CLICKED( ID_DISP_NOISE, &CFaceEditDispPage::OnButtonNoise )
	ON_BN_CLICKED( ID_DISP_SUBDIVIDE, &CFaceEditDispPage::OnButtonSubdivide )
	ON_BN_CLICKED( ID_DISP_SEW, &CFaceEditDispPage::OnButtonSew )
	ON_BN_CLICKED( ID_DISP_PAINT_GEO, &CFaceEditDispPage::OnButtonPaintGeo )
	ON_BN_CLICKED( ID_DISP_PAINT_DATA, &CFaceEditDispPage::OnButtonPaintData )
	ON_BN_CLICKED( ID_DISP_TAG_WALK, &CFaceEditDispPage::OnButtonTagWalkable )
	ON_BN_CLICKED( ID_DISP_TAG_BUILD, &CFaceEditDispPage::OnButtonTagBuildable )
	ON_BN_CLICKED( ID_DISP_TAG_REMOVE, &CFaceEditDispPage::OnButtonTagRemove)
	ON_BN_CLICKED( ID_DISP_INVERT_ALPHA, &CFaceEditDispPage::OnButtonInvertAlpha )
	ON_BN_CLICKED( IDC_SELECT_ADJACENT, &CFaceEditDispPage::OnSelectAdjacent )

	ON_NOTIFY( UDN_DELTAPOS, ID_SPIN_DISP_POWER, &CFaceEditDispPage::OnSpinUpDown )
	ON_NOTIFY( UDN_DELTAPOS, ID_SPIN_DISP_ELEVATION, &CFaceEditDispPage::OnSpinUpDown )
	ON_BN_CLICKED( ID_DISP_APPLY, &CFaceEditDispPage::OnButtonApply )
	//}}AFX_MSG_MAP
